...
module.exports = {
    GETDriverTaskList: async function(driverId) {
        ...
        for (let dataItem of dataList) {
            ...
            if (dataItem.dataFrom && dataItem.dataFrom == 'SYSTEM') {
				if (tempTaskId && tempTaskId.startsWith('AT-')) {
    ...
    GETDriverTaskByTaskId: async function(taskId) {
        ...
        let dataItem = dataList && dataList.length > 0 ? dataList[0] : null;
        if (dataItem) {
            let systemTask = null;
            if (dataItem.dataFrom == 'SYSTEM') {
                ...
                systemTask = systemTaskList && systemTaskList.length > 0 ? systemTaskList[0] : null;
            }
    ...
    getDriverNextTask: async function (req, res) {
        try {
            ...
            if (!user) {
                throw `GetDriverNextTask UserId ${ userId } do not exist.`
            }
            ...
            if (!currentTask) {
                throw `GetDriverNextTask TaskId ${ currentTaskId } do not exist.`
            }
