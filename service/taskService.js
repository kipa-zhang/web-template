...
module.exports = {
	...
	getWeatherForecast: async function (req, res) {
		...
		const getWeatherData = async function () {
			...
			if (data && data.api_info.status === 'healthy' && JSON.stringify(data.items[0]) !== '{}') {
				...
				throw 'Weather_Forecast_API is not working now.'
			}
		}
		try {
	...
	getDriverMileage: async function (req, res) {
		try {
			...
			if (!driver) {
				throw `DriverId ${user.driverId} do not exist.`
			}
				...
				for (let indent of result.data.indents) {
					...
					for (let task of indent.dataList) {
						...
						if (mileage) {
							...
							let vehicle = await Vehicle.findByPk(task.vehicleNumber)
							task.startMileage = (vehicle && vehicle.totalMileage) ? vehicle.totalMileage : 0;
							task.endMileage = 0;
						}
	...
	getTOIndentByTaskId: async function (req, res) {
		try {
			...
			if (result.code === 1) {
				...
				task.commanderContact = (mtRAC && mtRAC.needCommander) ? { username: mtRAC.commander, contact: mtRAC.commanderContactNumber, hub: assignedTask.hub, node: assignedTask.node } : null;
	...
	startTask2: async function (req, res) {
		try {
			...
			if (!taskId) {
				...
				throw `TaskId ${taskId} can not be null.`
			}
			...
			await sequelizeObj.transaction(async (t1) => {
				...
				if (!tempTask) {
					log.warn(`TaskId ${taskId} do not exist.`)
					throw `TaskId ${taskId} do not exist.`
				}
				...
				if (tempTask.dataFrom == 'SYSTEM') {
					...
					if (systemTaskId && systemTaskId.startsWith('AT-')) {
					...
					await sequelizeSystemObj.transaction(async (t1) => {
						...
						if (!sysTask) {
							...
							throw `SystemTask TaskId ${systemTaskId} do not exist.`;
						}
