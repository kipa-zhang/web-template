...
module.exports.updatePosition = async function (req, res) {
	const checkToken = async function(userId, token) {
		...
		if (!loginRecord) {
			...
			throw `Your account has been login at other place. Please login again.`
		}
		if (moment().diff(loginRecord.updatedAt) > 36 * 60 * 60 * 1000) {
			log.warn(`Mobile need to re-login now !!!!!!!!!!!!!!`)
			throw `Mobile need to re-login now !!!!!!!!!!!!!!`
		}
	}
...
module.exports.updateWaypointPosition = async function (req, res) {
	try {
		...
		sequelizeObj.transaction(async transaction => {
			...
			if (!vehicleRelation) throw `There is no vehicleRelation data by { driverId: ${ user.driverId }, vehicleNo: ${ vehicleNo } }`
			let driverTask = await DriverTask.findOne({ vehicleRelationId: vehicleRelation.id })
			if (!driverTask) throw `There is no driver { vehicleRelationId: ${ vehicleRelation.id } } task data.`
			let arrivedInfo = driverTask.arrivedInfo;
			if (!arrivedInfo) throw `There is no Arrived Info from DriverTask { driverTaskId: ${ driverTask.id } }`
    ...
    const operationPositionData = async function (positionList, res) {
		...
		const checkNoGPSSignal = function (driverPosition, positionList) {
			if (driverPosition && positionList.length) {
				...
				if (moment(driverPosition.gpsTime).format('YYYY-MM-DD HH:mm:ss') == moment(Number.parseInt(position.gpstime)).format('YYYY-MM-DD HH:mm:ss') ) {
					return true;
				} else {
					return false;
				}
        ...
        try {
			...
			await sequelizeObj.transaction(async transaction => {
				...
				if (driver.state?.toLowerCase().indexOf('pause') > -1) {
					...
					// So need check do nt change here.
					missingType = driver.state;
				} else {
					if (checkNoGPSSignal(latestDriverPosition, positionList)) {
						if (gpsService == 0) {
						...
						if (latestDriverPosition?.gpsPermission == 0) {
							// no useful here(No permission will not create data)
							missingType = 'No GPS Permission'
						} else if (driver.state?.toLowerCase().indexOf('pause') > -1) {
							// no useful here(Pause will not create data)
							missingType = 'Pause'
            ...
            gpsLog.error('operationPositionData: ', error)
            return res.json(utils.response(0, error));
        }
    }
}
